...
!=======================================================================
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    use cam_history,        only: addfld !+++WEC
    ...
    call addfld ( 'UTEND_NDG', (/ 'lev' /), 'A', 'm/s2', 'Zonal wind tendency by nudging') !+++WEC
    call addfld ( 'VTEND_NDG', (/ 'lev' /), 'A', 'm/s2', 'Meridional wind tendency by nudging') !+++WEC
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac (ztodt,   cam_in,  &
    ...
    use nudging,            only: Nudge_Model,Nudge_ON,nudging_timestep_tend, zero_nudging,First_Nudge,flip_first_nudge
   ...
   if (carma_do_aerosol) then
    ...
    if((Nudge_Model).and.(Nudge_ON)) then
      ...
      if(First_Nudge) then
          ...
          call physics_update(state,ptend,ztodt,tend)
          call check_energy_chng(state, tend, "nudging", nstep, ztodt, zero, zero, zero, zero)
          call flip_first_nudge(state)
      else
          !write(iulog,*) 'Nachos Nudge First Step = ',First_Nudge
          call nudging_timestep_tend(state,ptend)
    ...
    !place tolerance check here:
    !state = state_preperturb
